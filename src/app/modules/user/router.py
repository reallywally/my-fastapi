...
async def create_user(db: TxDep, obj: CreateUserRequest) -> UserResponse:
    user = await user_service.create(db=db, obj=obj)
    return UserResponse.model_validate(user)


@router.get('', summary='목록 (커서 페이지네이션)')
async def list_users(db: ConnDep, page: CursorDep) -> Page[UserResponse]:
    return await user_service.list(db=db, cursor=page.cursor, size=page.size)


@router.get('/{pk}', summary='상세')
async def get_user(db: ConnDep, pk: int) -> UserResponse:
    user = await user_service.get(db=db, pk=pk)
    return UserResponse.model_validate(user)


@router.patch('/{pk}', summary='수정 (본인 또는 관리자)')
async def update_user(db: TxDep, pk: int, obj: UpdateUserRequest, actor: PrincipalDep) -> UserResponse:
