"""연결 의존성 중 DB 가 필요 없는 부분. 실제 트랜잭션 동작은 integration 쪽에 있다."""

import pytest

from app.common.db import get_connection_source, get_db, get_db_tx
...
def test_missing_connection_source_raises():
    """§3.3 — 자원이 없으면 예외지, 프로세스 종료가 아니다."""
    with pytest.raises(RuntimeError, match='lifespan'):
        get_connection_source(_RequestWithoutState())
