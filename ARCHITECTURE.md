...
- [x] `board/comment/` — `path` 기반 트리, 깊이 1단 제한 (§4.2)
...
- **읽기 권한 판정은 `board` 슬라이스가 소유한다.** `/posts/{id}` 와
  `/posts/{id}/comments` 는 `slug` 를 거치지 않는 평평한 경로라 `require_board` 가
  붙지 않는다. 규칙을 두 벌로 만들면 한쪽만 고치는 날이 오고, 그날 비공개 게시판이
  열린다 — `board_service.readable()` 하나를 세 곳이 부른다
- **댓글 트리의 커서는 `path` 문자열이다.** 정렬 키가 곧 커서라야 keyset 이 성립한다.
  `Page[T]` 의 커서를 `int | str` 로 넓히지 않고 `CommentPageResponse` 를 따로 뒀다 —
  모양(`{items, next_cursor, has_next}`)은 같으니 화면이 다루는 방식은 그대로다
- **묘비는 `comment_count` 를 줄이지 않는다.** 화면에 자리가 남아 있으면 그것은 여전히
